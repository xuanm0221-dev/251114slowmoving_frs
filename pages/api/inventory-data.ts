...
export default async function handler(
  ...
  try {
    // 전처리 JSON에서만 읽기
    console.log(`[inventory-data] 전처리 JSON에서 데이터를 읽습니다. brand=${brand}, referenceMonth=${refMonth}`);
    
    let jsonData: any;
    try {
      jsonData = readBatchJsonFile<any>("accessory_inventory_summary.json");
    } catch (error) {
      console.error(`[inventory-data] JSON 파일 읽기 실패:`, error);
      // JSON 파일이 없으면 빈 데이터 반환
      const emptyData: InventoryAPIResponse = {
        brands: {
          [brand]: {
            전체: {},
            Shoes: {},
    ...
    const filteredBrandData: InventoryBrandData = {
      전체: {},
      Shoes: {},
      Headwear: {},
      Bag: {},
    ...
    const filteredData: InventoryAPIResponse = {
      brands: {

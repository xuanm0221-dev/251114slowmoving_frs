...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // baseMonth = 현재 기준월 (대리상 API는 단월 조회이므로 baseMonth = referenceMonth)
    const liveData = await fetchDealerDataFromSnowflake(brand, baseMonth, priorMonth, daysInMonth, dealerNames, baseMonth);

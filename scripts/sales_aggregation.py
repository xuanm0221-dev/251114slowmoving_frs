...
def build_sales_aggregation_query(
...
def aggregate_sales_from_snowflake(
    ...
    print(f"[판매] Snowflake에서 데이터 조회 중... ({start_month} ~ {end_month}), 기준월(ref)={ref}")

    query = build_sales_aggregation_query(start_month, end_month, ref)
